...
def reset_engine() -> None:
    ...
    用于 Celery worker 等场景：每次 asyncio.run() 会创建新的 event loop，
    而旧引擎的连接池绑定在已关闭的旧 loop 上，必须丢弃重建。

    通过 sync_engine.dispose() 同步关闭连接池中的所有 DB 连接，
    避免因 GC 无法 await 异步 close() 而泄漏 TCP 连接。
    """
