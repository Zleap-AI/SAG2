...
class EventVectorRepository(BaseRepository):
    ...
    async def get_events_by_ids(
        ...
        query_body = {
            ...
            "size": len(event_ids),
            "_source": {
                "excludes": ["title_vector"]  # 排除 title_vector 减少 ES 压力
            }
        }
    ...
    async def search_similar_by_content(
        ...
        Args:
            ...
            start_time: 时间范围起点（可选）
            end_time: 时间范围终点（可选）
            source_ids: 事项来源ID列表（Article/Conversation ID，可选）
    ...
    async def _vector_search(
        ...
        # 时间过滤：
        # 1) 有 end_time: 使用区间重叠
        # 2) 无 end_time: 仅要求 start_time 在查询范围内
    ...
    def _build_time_range_filter(
        ...
        """
        构建 ES 时间范围过滤条件（区间重叠语义）。

        注意：逻辑与 pipeline.modules.search.time_filter.build_es_time_range_filter 一致，
        此处为本地副本以避免循环导入（event_repository → modules.search → event_repository）。
        """
    ...
    async def search_by_time_range(
        ...
        """
        按时间范围搜索（区间重叠语义：返回与查询时间段有交集的事件）

        对于有 end_time 的事件，使用区间重叠判断；
        对于没有 end_time 的事件，判断 start_time 是否落在查询范围内。
