...
class ElasticsearchClient:
        ...
        **kwargs: Any,
        ...
        if hosts:
            ...
            if settings.es_username and settings.es_password:
                ...
                client_config = {
                    "hosts": [auth_url],
                    "request_timeout": default_timeout,  # 添加默认超时
                    **kwargs,
                }
            else:
                client_config = {
    ...
    def __getattr__(self, name: str):
        """
        委托未定义的方法给底层的 AsyncElasticsearch 客户端

        这样 ElasticsearchClient 可以同时提供：
        - 自定义方法（如 vector_search, index_document, get_document 等）
        - AsyncElasticsearch 的原生方法（如 search, index, get, delete 等）

        这使得 Repository 可以统一使用 ElasticsearchClient 对象，既能调用自定义方法，
        也能调用原生方法。
        """
        ...
        **kwargs: Any,
        ...
        try:
            search_params = {
                ...
                "size": size,
                "from_": from_,
                "timeout": "300s",  # 添加查询级别超时，避免高并发时无限等待
                **kwargs,
            }
            if routing:
    ...
    async def vector_search(
        ...
        Args:
            ...
            filter_query: 过滤条件
            routing: 路由键（可选，用于指定分片）
            include_vector: 是否返回向量字段（默认 False，减少约4KB/条的数据传输）
        ...
        try:
            knn_query: Dict[str, Any] = {
                ...
                "query_vector": vector,
                "k": size,
                "num_candidates": max(100, size * 2),  # 固定候选数量，k不会超过100
            }
            ...
            search_params = {
                ...
                "knn": knn_query,
                "size": size,
                "timeout": "300s",  # 添加查询级别超时，避免无限等待
            }

            # 根据参数决定是否排除 vector 字段
...
def reset_es_client() -> None:
    ...
    用于 Celery worker：每次 asyncio.run() 创建新 event loop，
    旧客户端内的 aiohttp session 绑在已关闭的旧 loop 上，必须丢弃。
    """
    global _es_client
    _es_client = None
